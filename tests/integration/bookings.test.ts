...
describe("POST /booking", () => {
  ...
  describe("when token is valid", () => {
    ...
    it("should respond with status 403 when the user already has a booking", async () => {
      ...
      const hotel = await createHotel();
      const room = await createRoom(hotel.id);
      const roomReserved = await createRoom(hotel.id);
      const body = {
        roomId: room.id
      };

      await createBooking(user.id, roomReserved.id);         
      const response = await server.post("/booking").set("Authorization", `Bearer ${token}`).send(body);
...
describe("PUT /booking", () => {
  ...
  describe("when token is valid", () => {
    it("should respond with status 403 when the user does not have a booking or the booking is not the user's", async () => {
      const user = await createUser();
      const token = await generateValidToken(user); 
      const enrollment = await createEnrollmentWithAddress(user);
      const ticketType = await createTicketTypeWithHotel(false, true);
      ...
      const booking = await createBooking(user.id+1, roomReserved.id);
    ...
    it("should respond with status 403 when the room has no vacancy", async () => {
      ...
      const booking = await createBooking(user.id, roomReserved.id); 
      await createBooking(user.id+1, room.id); 
    ...
    it("should respond with status 200 and with booking id", async () => {
      ...
      const booking = await createBooking(user.id, room.id+1);
      const response = await server.put("/booking"+booking.id).set("Authorization", `Bearer ${token}`).send(body);
      const exchangedBooking = await prisma.booking.findFirst({ where: { userId: user.id } });
      expect (exchangedBooking.roomId).toEqual(room.id);
      expect(response.status).toEqual(httpStatus.OK);
      expect(response.body).toEqual({
